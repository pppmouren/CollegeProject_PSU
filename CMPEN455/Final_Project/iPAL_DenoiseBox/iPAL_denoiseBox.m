...
handles.denoised_mode_DONE = 'N/A';
handles.sigma_DONE = '0';
...
set(handles.Level_Type_display,'Visible','On');
contents = cellstr(get(hObject,'String'));
...
if handles.denoised_done
    ...
    handles.denoised_mode_DONE = handles.methodsBook{handles.denoise_mode};
    handles.sigma_DONE = num2str(handles.record_sigman);
...
handles.denoised_done = false;
set(handles.mode_display,'Visible','On');
set(handles.Level_Type_display,'Visible','Off');
guidata(hObject, handles)                              % update the handles
...
figure();
imshow(handles.denoised_image_passing,[0,255]),title(['Denoised image. Mode=',handles.denoised_mode_DONE],'FontSize',15);
...
imshow(uint8(wcodemat(wcodemat(handles.denoised_image_passing,255) - double(handles.original_image),255))),...
    title(['Difference Between Denoised Image and Original Image. Mode=',handles.denoised_mode_DONE],'FontSize',15);
...
elseif strcmp(handles.passingShowIndex,'D')
    ...
    imshow(decomposition,[0,255]),title(['Denoised Image wavelet transform of ', handles.wavelet_type, ' with ', num2str(handles.level_num),' levels Mode=',handles.denoised_mode_DONE],'FontSize',15);
end
...
elseif strcmp(handles.passingShowIndex,'D')
    ...
    imshow(F2,[]),title(['Denoised Image 2D DFT with Mode=',handles.denoised_mode_DONE],'FontSize',15);
end
...
elseif strcmp(handles.passingShowIndex,'D')
    ...
    F2=log(1+fftshift(abs(F)));
    figure('Name','Denoised Image 2D DFT');
    mesh(F2),title(['Denoised Image 2D DFT with Mode=',handles.denoised_mode_DONE],'FontSize',15);
end
...
elseif strcmp(handles.passingShowIndex,'N')
    figure('Name','Noisy Image'),imshow(handles.noisy_image,[0,255]),title(['Noisy image with sigman=', handles.sigma_DONE],'FontSize',15);
elseif strcmp(handles.passingShowIndex,'D')
    figure('Name','Denoised Image'),imshow(handles.denoised_image_passing,[0,255]),title(['Denoised image. Mode=',handles.denoised_mode_DONE],'FontSize',15);
