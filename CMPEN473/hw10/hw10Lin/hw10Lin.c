/**************************************************
* File:  hw10Lin.c 
* Homework 10, manual car driving, and camera base black line tracing 
* By Xuhong Lin
* CMPEN 473, Spring 2024, Penn State University
* 
* This is a Homework 10, we will use command to control car driving
* Two Modes: 'm1' mode for manual driving, 'm2' mode for black line tracing 
...
* 2. "m2" mode:
 ...
 (When m2 mode is entered, start moving the car to search for the track line. When ‘w’ key hit, then follow
the track line, tracing)
* 
...
* 4. USB receive the camera image data for black line tracing
* 
* QUICK NOTE:
* 1. always display on a terminal window the prompt ‘Hw10>' and display
...
* 3. m2 mode:
* 		3.1. In m2 mode, same rule as hw6, but we get the inforamtion from image instead of IR senser
*		3.2. Car autonomously follows a black line on the white board using a camera;
*		3.3. Car moves at full speed but it may move slower on the curbed line, never stopping in any case;
*		3.4. Able to overcome small line discontinuity and misalignment, for the track is drawn on the multiple 
*			 white boards.
* 
...
#define m1Mode 1
#define m2Mode 2
#define imageScaleFactor 20
#define APB_CLOCK 250000000
#define ROUND_DIVISION(x,y) (((x) + (y)/2)/(y))
#define Acce_factor 1.00
#define AcceZ_factor 10.00
#define Gyro_factor 50.00
#define Time_Interval_S 0.01
#define m2InitSpeed 40
#define m2TurnSpeed 40
#define m2MaxSpeedDiff 30

uint32_t rightPWMLevel = 50;    /* right wheeql PWM as duty cycle, set default to 50 */
    ...
    FILE* fd)
{
  ...
  accelBufferX[printIndex] = 5 + ACCEL_X;
  accelBufferY[printIndex] = 5 + ACCEL_Y;
  accelBufferZ[printIndex] = 5 + ACCEL_Z;
  gyroBufferX[printIndex] = 5 + GYRO_X;
  gyroBufferY[printIndex] = 5 + GYRO_Y;
  gyroBufferZ[printIndex] = 5 + GYRO_Z;
  printIndex++;
  if(printIndex == 60000){
...
{	
	...
	if (currMode == m1Mode){
		...
		// code here
		//car move straight
		if (pointX >= 15 && pointX <= 16)
		{
			// set direction to forward
			param->pwm->DAT1 = m2InitSpeed + 7;
		}
		// car move left
		else if (pointX < 15 && pointX >=0)
		{
			float speedDiff = m2MaxSpeedDiff*(1-pointX/15);
			param->pwm->DAT1 = m2TurnSpeed - (int)speedDiff;
			//printf("left motor: left turn: %d\n", param->pwm->DAT1);
		}
		// car move right
		else if (pointX > 16 && pointX <=31)
		{
			float speedDiff = m2MaxSpeedDiff*((pointX-16)/15);
			param->pwm->DAT1 = m2TurnSpeed + (int)speedDiff + 20;
			//printf("left motor: lerightft turn: %d\n", param->pwm->DAT1);
		}
...
void *right_wheel_control(void *arg)
{
	...
	if (currMode == m1Mode){
		...
		{
			// set direction to forward
			param->pwm->DAT2 = m2InitSpeed;
		}
		// car move left
		else if (pointX < 15 && pointX >=0)
		{
			float speedDiff = m2MaxSpeedDiff*(1-pointX/15);
			param->pwm->DAT2 = m2TurnSpeed + (int)speedDiff;
			//printf("right motor: left turn: %d\n", param->pwm->DAT2);
		}
		// car move right
		else if (pointX > 16 && pointX <=31)
		{
			float speedDiff = m2MaxSpeedDiff*((pointX-16)/15);
			param->pwm->DAT2 = m2TurnSpeed - (int)speedDiff - 7;
			//printf("right motor: right turn: %d\n", param->pwm->DAT2);
		}
	}
    }
    return NULL;
...
void *image_process_control(void *arg)
{
	...
	while (!Done)
	{
		...
		int grayScaledImg[param->scaled_height][param->scaled_width];
		int sampleWindowHeight = param->scaled_height / 4;
		...
		// code here
		// Process imge
		// Step 1: white background, try to only show the black line
		//         reverse the white s black, black as white
		for (int i = 0; i < param->scaled_height; i++)
		{
			for (int j = 0; j < param->scaled_width; j++)
			{
				if(redLayer[i][j] < 60 && greenLayer[i][j] < 60 && blueLayer[i][j] < 60)
				{
					//set potential balck line to 255
					grayScaledImg[i][j] = 255;
					param->scaled_RGB_data[i*param->scaled_width+j].R = 255;
					param->scaled_RGB_data[i*param->scaled_width+j].B = 255;
					param->scaled_RGB_data[i*param->scaled_width+j].G = 255;
					
				}
				else
				{	
					grayScaledImg[i][j] = 0;
					param->scaled_RGB_data[i*param->scaled_width+j].R = 0;
					param->scaled_RGB_data[i*param->scaled_width+j].B = 0;
					param->scaled_RGB_data[i*param->scaled_width+j].G = 0;
				}
			}
		}
		
		#if 0
		printf("gray scaled image\n");
		for (size_t i = 0; i < param->scaled_height; i++)
		{
		    for (size_t j = 0; j < param->scaled_width; j++)
		    {
				printf("%3d ", grayScaledImg[i][j]);
		    }
		    printf("\n");
		}
		#endif
		
		// Step 2: sample out the botton of the matrix
		int sampledImg[sampleWindowHeight][param->scaled_width];
		for (int i = 0; i < sampleWindowHeight; i++)
		{
			for (int j = 0; j < param->scaled_width; j++)
			{
				sampledImg[i][j] = grayScaledImg[i + sampleWindowHeight*3][j];
			}
		}
		...
		for (int i = 0; i < sampleWindowHeight; i++)
		{
			for (int j = 0; j < param->scaled_width; j++)
			{
				printf("%3d ", sampledImg[i][j]);
			}
			printf("\n");
		}
		#endif
		
		// Step 3: based on the sample matrix, try to get the center of gravity of the matrix(COG)
		//		   only calculate the x axis to located position
		int total = 0;
		int half_total = 0;
		for (int j = 0; j < param->scaled_width; j++)
		{
			total += sampledImg[2][j];
		}
		for (int k = 0; k < param->scaled_width; k++)
		{
			half_total += sampledImg[2][k];
			if(half_total > (int)total/2){
				pointX = k;
				//printf("the X point is %d\n", k);
				break;
			}
		}
...
int main(int argc, char * argv[] )
{
	...
	FILE *fd_m0 = fopen("hw10m0data.txt", "w");
	FILE *fd_m1 = fopen("hw10m1data.txt", "w");
	FILE *fd_m2 = fopen("hw10m2data.txt", "w");
    ...
    printf("There are two modes for use, 'm1': manual driving mode; 'm2': self-drive black line tracing with camera\n");
    ...
    printf("(10) 'p': print data\n");
    printf("(11) 'n': show total distance and average speed\n");
    printf("Under the 'm2' mode, we have commands listed below\n");
    printf("(1) 'q': to quit all program\n");
    printf("(2) 'm1': change mode to m1\n");
    printf("(3) 'w': to start the tracing\n");
    printf("(4) 's': stop\n");
    printf("(5) 'p': print data\n");
    printf("(6) 'n': show total distance and average speed\n");
