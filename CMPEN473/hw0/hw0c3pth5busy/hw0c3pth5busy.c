/**************************************************
* File:  hw0c3pth5busy.c 
* Homework 4, pthread Sample 5 Program 
* By Kyusun Choi
...
* Watch the Raspberry Pi 4 processor core CPU utilation with 'htop' command
* compare with the Homework 4, pthread Sample 4 Program hw0c3pth5busy.c
* 
* This is a Homework 4, pthread Sample 5 Program, two LED blinking in 2 threads.
*  >>> using "busy wait" rather than "sleep" <<<  demonstrate CPU utilization
...
*  This is the 5th sample program, showing two threads for two LED blinking
*    using "busy wait" which will make CPU very busy.
*    Also good example pthread parameter, pthread_create, and pthread_join. 
*  This program:
...
void dly4500us()   // busy delay, approximately 5msec on Raspberry Pi 4 computer.
{
  int i = 1;
  while(i <= 1000000)
  {
...
{
  int i = 1;
  while(i <= 108)     // 108 => 0.5sec, busy delay
  {
    dly4500us();
    ++i;
  }
