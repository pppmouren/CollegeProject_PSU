...
static bool nread(int fd, int len, uint8_t *buf) {
  ...
  //loop through to read all buffer
  while(cur_len < len){
    return_len = read(fd, buf + cur_len, len - cur_len);
    cur_len += return_len;
  }
  return true;
}
...
static bool nwrite(int fd, int len, uint8_t *buf) {
  int cur_len = 0;
  int return_len = 0;
  //loop through to write all buffer
  while(cur_len < len){
    return_len = write(fd, buf + cur_len, len - cur_len);
    cur_len += return_len;
  }
  return true;
}
...
static bool recv_packet(int sd, uint32_t *op, uint8_t *ret, uint8_t *block) {
  ...
  if(nread(sd, HEADER_LEN, header_buf) == false){
    return false;
  }
  //copy op and ret
  memcpy(op, header_buf, 4);
  memcpy(ret, &header_buf[HEADER_LEN - 1], 1);
  //determine to read block or not
  if((*ret & 0x02) == 2){
...
static bool send_packet(int sd, uint32_t op, uint8_t *block) {
  ...
  //convert op to net format and put n_op to Header buffer
  uint32_t n_op = htonl(op);
  uint8_t ret = 0x00;
  
  //put n_op to packet
  memcpy(packet, &n_op, 4);

  //if op is write, block contains data, need to pass to packet
  if(((op >> 12) & 0x3f) == JBOD_WRITE_BLOCK){
    //put info byte to Header
    ret = 0x02;
    //put info code to packet
    memcpy(&packet[HEADER_LEN - 1], &ret, 1);
  ...
  //if op is not write, just sent header to sd
  else{
    memcpy(&packet[HEADER_LEN - 1], &ret, 1);
 
    if(nwrite(sd, HEADER_LEN, packet) == false){
      return false;
    }
  }
...
int jbod_client_operation(uint32_t op, uint8_t *block) {
  ...
  recv_packet(cli_sd, &raw_op, &ret, block);
