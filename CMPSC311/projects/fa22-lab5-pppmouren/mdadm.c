...
void seek_func(uint32_t seektoblock_op, uint32_t seektodisk_op){
  jbod_client_operation(seektodisk_op, NULL);
  jbod_client_operation(seektoblock_op, NULL);
}
...
int mdadm_mount(void) {
  ...
  if (jbod_client_operation(mount_op,NULL) == 0){
    is_mounted = 1;
    return 1;
  }
  else{
...
int mdadm_unmount(void) {
  ...
  if (jbod_client_operation(unmount_op,NULL) == 0){
    is_mounted = 0;
    return 1;
  }
  else{
...
int mdadm_write_permission(void){
  ...
  if(jbod_client_operation(write_permission_op, NULL) == 0){
     is_written = 1;
     return 1;
  }
  else{
...
int mdadm_revoke_write_permission(void){
  ...
  //check if write permission working
  if(jbod_client_operation(revoke_write_permission_op, NULL) == 0){
     is_written = 0;
     return 1;
  }
  else{
...
int mdadm_read(uint32_t start_addr, uint32_t read_len, uint8_t *read_buf)  {
  ...
  while (remain_len > 0){
    ...
    if(cache_enabled() == true){
      ...
      if(a == -1){
	jbod_client_operation(read_op, temp_buf);
...
int mdadm_write(uint32_t start_addr, uint32_t write_len, const uint8_t *write_buf) {
  ...
  while(remain_len > 0){
    ...
    // write mem and cache
    seek_func(seek_block_op, seek_disk_op);
    jbod_client_operation(write_op, temp_buf);
    if(a == 1){
